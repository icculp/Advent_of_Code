...
for line in numbers:
    ...
    if x1 == x2:
        ...
        print('hereeeee')
        print(crossed[x1, np.minimum(y1, y2): np.maximum(y1, y2) + 1].shape)
        # break
    elif y1 == y2:
        crossed[np.minimum(x1, x2): np.maximum(x1, x2) + 1, y1] += 1
        print('herre')
        print(crossed[np.minimum(x1, x2): np.maximum(x1, x2) + 1, y1].shape)
        #break
    else:
        ...
        if x2 > x1 and y2 > y1:
            ...
            if x1 > x2:
                if y1 > y2:
                    ...
                    print(line)
                    print(y1, y2)
                    print(crossed[np.minimum(x1, x2): np.maximum(x1, x2) + 1, np.minimum(y1, y2): np.maximum(y1, y2) + 1].shape)  
                    crossed[np.minimum(x1, x2): np.maximum(x1, x2) + 1, np.minimum(y1, y2): np.maximum(y1, y2) + 1] += diag
                else:
                    #print(x1, x2, y1, y2)
...
for (x, y, X, Y) in numbers:
    dxzzzzzzzzzzzzzzzzzzzzzzzz, dy = np.sign([X-x, Y-y])                 
    while (x,y) != (X+dx, Y+dy):
        grid[dx * dy, x, y] += 1
        x+=dx; y+=dy
